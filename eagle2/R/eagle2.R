...
#' @param fixed_conc Optional fixed K-vector of concentration parameters
#' @param concShape Shape of prior on concentration
#' @param concRate Rate of prior on concentration
...
#' @export
eagle2=function(ys,ns,fixed_conc=NULL,concShape=1.0001,concRate=1e-4,hessian=T,...) {
  ...
  stan_model_to_use=if (is.null(fixed_conc)) stanmodels$bb else stanmodels$bb_fixed_conc

  xNull=preprocess_x( foreach(i=seq_len(Ti)) %do% matrix(1,N,1) )
  dat=list(N=N,P=1,T=Ti,K=K,ys=ys,ns=ns,x=xNull,
           concShape=concShape,concRate=concRate,
           conc=if(is.null(fixed_conc)) NULL else as.array(fixed_conc))

  # Fit null model
  fit_null <- optimizing(stan_model_to_use, data=dat, as_vector=F, hessian=T, ...)
  ...
  datFull$P=Ti
  fit_full <- optimizing(stan_model_to_use, data=datFull, init=initFull, as_vector=F, hessian=T, ...)
